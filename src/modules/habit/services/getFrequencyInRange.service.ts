...
@Injectable()
export default class GetFrequencyInRangeService {
  constructor(
  ...
  async execute({ userId, habitId, initalDate, finalDate }): Promise<any> {
    try {
      ...
      const registerChecks = await this.daysCheckRepository.find({
        where: {
          user_id: userId,
          habit_id: habitId,
          date: Between(initalDate, finalDate),
        },
      });

      const dates = this.getDates(initalDate, finalDate);
